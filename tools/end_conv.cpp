...
int main(int argc, const char * argv)
{
	...
	for (;;)
	{
		...
		//
		if ((isxdigit(s[0]) || s[0] == ' ') && (isxdigit(s[1]) || s[1] == ' ') && (isxdigit(s[2]) || s[2] == ' ') && isxdigit(s[3]) && s[4] == ':')   // '  xx:'
		{
		...
		else if (isxdigit(s[0]) && isxdigit(s[1]) && isxdigit(s[2]) && isxdigit(s[3]) && isxdigit(s[4]) && isxdigit(s[5]) && isxdigit(s[6]) && isxdigit(s[7]))             // 'xxxxxxxx'
		{
			assert(1 == sscanf(s, "%x", &pc));
		...
		// endian swap.
		//
		while (isxdigit(s[0]) && isxdigit(s[1]) && s[2] == ' ' && isxdigit(s[3]) && isxdigit(s[4]) && s[5] == ' ')
		{
